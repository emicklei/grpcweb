...
func TestNonRootResource(t *testing.T) {
	...
	req := httptest.NewRequest("OPTIONS", "http://host/grpc/test.TestService/Echo", nil)
	req.Header = headers
...
func (s *GrpcWebWrapperTestSuite) TestCORSPreflight_DeniedByDefault() {
	...
	*/
	...
	corsResp, err := s.makeRequest("OPTIONS", "/test.TestService/PingList", headers, nil, false)
...
func (s *GrpcWebWrapperTestSuite) TestCORSPreflight_AllowedByOriginFunc() {
	...
	*/
	...
	corsResp, err = s.makeRequest("OPTIONS", "/test.TestService/Unknown", headers, nil, false)
	assert.NoError(s.T(), err, "cors preflight should not return errors")
...
func (s *GrpcWebWrapperTestSuite) TestCORSPreflight_EndpointsOnlyTrueWithHandlerFunc() {
	...
	*/
	...
	const pingMethod = "/test.TestService/PingList"
	const badMethod = "/test.TestService/Bad"
